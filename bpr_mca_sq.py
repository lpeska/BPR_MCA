"""
Bayesian Personalized Ranking with Content Alignment

Matrix Factorization model and a variety of classes
implementing different sampling strategies.
Original Implementation of BPR and sampling strategies from github.com/gamboviol/bpr

Extended with Content Alignment by Ladislav Peska, peska@ksi.mff.cuni.cz
"""
...
class BPR_MCA_sq(object):
    ...
    def __init__(self,args):
        ...
        D: number of factors
        data: sparse matrix of known data (used to tune regularization parameters)
        """
        ...
        for i in self.sim_matrix_names:
            self.sim_mat[i] = load_matrix(i, "data",self.shape,args["user_indicator"][j]) #create function load_matrix!!!
            self.sim_user[i] = args["user_indicator"][j]
            self.sim_lambda[i] = args["init_lambda"][j]
            #self.sim_mat[i] = self.sim_mat[i][0:100, 0:100]
            j = j+1
    ...
    def evaluate_hyperpar_results(self, aupr_val, auc_val, ndcg_val, iteration):
        hyperpar_metric =  ndcg_val; #aupr_val + auc_val +
    ...
    def fix_model(self, W, intMat, seed):
        ...
        #self.train(data,sampler, uSim, iSim, seed)

    #def train(self,data,sampler, uSim, iSim, seed):    
    def train(self, seed, test_data, test_label, hyperpar_search = False):
        ...
        for it in xrange(self.max_iters):
            ...
            #print (self.user_factors[1,:])   
            #print (self.item_factors[1,:]) 
    ...
    def update_factors(self,u,i,j,update_u=True,update_i=True):
        ...
        # update bias terms
        """if update_u:
            #user bias snizuje AUPR
            d = z - self.bias_regularization * self.user_bias[u]
            self.user_bias[u] += self.learning_rate * d
        ...
        #tady je treba vylepsit: vybrat jen uzivatele podobne k aktualnimu dle kazde metriky!!!!
        ##############new version################
    ...
    def _uniform_user_sampling(self, n_samples):
        ...
        for sgd_user in sgd_users:
            ...
            #neg_item = np.random.choice(list(self.train_objects - set(self.data[sgd_user].indices)))
    ...
    def complexity(self):
        ...
        for u,i,j in self.loss_samples:
            ...
            #check for validity, not sure whether it fits punctually
    ...
    def predict(self,u,i):
        ...
        #predict only from learned factors, ignore user and item similarity
        if self.simple_predict:
            return self.item_bias[i] + self.user_bias[u] + np.dot(self.user_factors[u],self.item_factors[i])
        #pokud nebyl uzivatel vubec mezi trenovanymi uzivateli, definuju jeho faktory na zaklade podobnosti
        elif (u not in self.train_users):   # & (i in self.train_objects)
            sum_sim = 0   
            ...
            """ 
            elif (i not in self.train_objects) & (u in self.train_users):     
                sum_sim_i = 0   
                vector_i = np.zeros((1,self.D))
                for k in self.sim_matrix_names:
                    if self.sim_user[k] == False:
                        sm = self.sim_mat[k]
                        items = np.nonzero(sm[i,:])[1]
                        factors = self.user_factors[items,:] 
                        dot_product = np.dot(sm[i,items], factors)
                        sum_sim_i += self.sim_lambda[k] * np.sum(sm[i,items])                
                        vector_i += self.sim_lambda[k] * dot_product 

                alignmentVectorI = vector_i/sum_sim_i            
            ...
            #jak item tak user jsou neznami
            elif (i not in self.train_objects) & (u not in self.train_users):
                sum_sim = 0   
                vector = np.zeros((1,self.D))          
                sum_sim_i = 0   
                vector_i = np.zeros((1,self.D))
    ...
    #need to rewrite this, brought from NRLMF    
    def evaluation(self, test_data, test_label, hyperpar_search = False):
        scores = []
                #similarities to the uknown drugs/targets are rather increasing noise, omit them
